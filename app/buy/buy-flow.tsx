...
export function BuyFlow({ paymentsEnabled }: { paymentsEnabled: boolean }) {
  ...
  // Live FX rates — the same numbers /api/checkout charges at, so the summary
  // shown here can never drift from the price the server computes.
  ...
  // Arc mainnet — the only payment network. Payments are refused server-side
  // when no receiver is configured (see lib/chains.ts); `paymentsEnabled`
  // mirrors that here so this page can't promise something the server won't
  // accept.
  ...
  /**
   * The order quote: null until live rates load, and null when this currency
   * can't be priced. Nothing downstream may charge without it, so a currency
   * we can't price shows no total instead of a plausible-looking guess.
   */
  const quote = useMemo(() => {
    if (!rates || !Number.isFinite(amountLocal)) return null;
    const subtotal = toUsd(amountLocal, country.currency, rates);
    if (subtotal === null) return null;
    const fee = platformFee(subtotal);
    return { subtotal, fee, total: round2(subtotal + fee) };
  }, [rates, amountLocal, country.currency]);

  const simulatedService = isSimulatedService(service);

  const ready =
    recipientValid && amountValid && Boolean(provider) && quote !== null && paymentsEnabled;
  ...
  async function runWalletPayment(chosen?: DetectedWallet) {
    ...
    try {
      ...
      // 2) Make sure we're on Arc (adds the chain to the wallet if needed).
      setPayStage(`Switching to ${payChain.label}…`);
                  ...
                  >
                    ...
                    <span>
                      ...
                      {s.delivery === "simulated" && (
                        <span className="mt-1 inline-block border border-ink-950 bg-sun-50 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-sun-800">
                          Simulated delivery
                        </span>
                      )}
                    </span>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="glass-strong p-6">
              ...
              <div className="mt-5 space-y-2 pt-4 text-sm">
                <Row label="Subtotal" value={quote ? formatUsd(quote.subtotal) : "—"} mono />
                <Row label="Platform fee (1.5%)" value={quote ? formatUsd(quote.fee) : "—"} mono />
                ...
                <p className="pt-1 text-xs font-medium leading-relaxed text-ink-500">
                  {rates ? (
                    <>
                      Charged in USDC · live rate 1 USD ≈{" "}
                      <span className="font-mono font-bold text-ink-600">
                        {effectiveRate(rates[country.currency] ?? 0).toLocaleString("en-US", {
                          maximumFractionDigits: country.currency === "NGN" ? 0 : 2,
                        })}{" "}
                        {country.currency}
                      </span>{" "}
                      (mid-market + {(FX_SPREAD * 100).toFixed(0)}%)
                    </>
                  ) : ratesError ? (
                    "Live exchange rates are unavailable right now, so we can't quote a price. Please try again in a few minutes."
                  ) : (
                    "Loading the live exchange rate…"
                  )}
                </p>
              </div>
              ...
              {txRef && (
                <div className="mt-4 border-2 border-ink-950 bg-sun-50 px-4 py-3.5 text-sm text-sun-800 animate-fade-in">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <a
                      href={payChain.explorerTx(txRef.txHash)}
                      target="_blank"
              ...
              {!paymentsEnabled && (
                <div className="mt-4 border-2 border-ink-950 bg-red-50 px-4 py-3 text-sm font-semibold leading-relaxed text-red-700 animate-fade-in dark:bg-red-500/15 dark:text-red-400">
                  <p className="font-bold">Payments are temporarily unavailable.</p>
                  <p className="mt-1">
                    We can&apos;t take USDC on Arc at the moment, so checkout is disabled.
                    Please try again later.
                  </p>
                </div>
              )}

              {simulatedService && paymentsEnabled && (
                <p className="mt-4 border-2 border-ink-950 bg-sun-50 px-4 py-3 text-xs font-semibold leading-relaxed text-sun-800">
                  {SIMULATED_DELIVERY_NOTICE}
                </p>
              )}
              ...
              >
                {loading ? (
                  ...
                  <>Pay {formatUsd(quote.total)} with USDC</>
                ) : txRef ? (
                  "Payment sent. Check again above"
                ) : !paymentsEnabled ? (
                  "Payments are unavailable right now"
                ) : recipientValid && amountValid && !quote ? (
